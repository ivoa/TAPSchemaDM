...
## Generating a TAPSchema instance from exiting Database

It is possible to generate a skeleton tap schema instance from an existing database using the 'all' jar variant that is available from the repository. The following

```shell

java -jar tapschema-0.9.9-all.jar --jdbc-url jdbc:postgresql://localhost:54771/quarkus --user user --password password      

```

will emit the tapschema to stdout - This can then be edited to include the human readable descriptions of the tables and columns. This full tapschema instance can then be used to generate the DDL for the model tables and the DDL to fill the TAPSchema tables with the model descriptions.

Note that the "all" jar only includes JDBC connectors for postgres by default. It is possible to add others to the command line by adding the JDBC driver jar to the classpath. For example, to add MySQL support, download the MySQL JDBC driver jar and then run

```shell
java --cp tapschema-0.9.9-all.jar:mysql-connector-java-8.0.33.jar org.ivoa.dm.tapschema.utils.SchemaReader --jdbc-url jdbc:mysql://localhost:3306/test --user user --password password
```